...
class pgwebw(WebKit2.WebView):
    ...
    def on_rmformat(self, action):
        ...
        if not ddd:
            return
        #print("Inserting", ddd)
        self.run_javascript("document.execCommand('insertText', null, '%s');" % ddd)
    ...
    def on_edit_marker(self, action):
        ...
        ccc = c.wait_for_contents(target)
        if not ccc:
            print("No data on clipboard")
            self.xlink.set_status("No data on selected.")
            return

        #print("sel", ccc.get_data())
        #ccc.free()
        ddd = ccc.get_data().decode()
        htmlx = markdialog(ddd)
        if not htmlx:
            htmlx = ddd
        #Gtk.Clipboard.clear(c)
        print("markdialog result:\n", htmlx)
        self.run_javascript("document.execCommand('insertHTML', null, '%s');" \
                                    % htmlx)
    ...
    ("removeformat", Gtk.STOCK_PROPERTIES, "_removeFormat", "<Control>M", "Remove Formatting", self.on_rmformat),
